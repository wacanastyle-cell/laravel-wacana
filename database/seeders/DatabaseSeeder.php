...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $memberRegistrationForm = Form::firstOrCreate([
            'slug' => 'member-registration',
        ], [
            'title' => 'Formulir Pendaftaran Member Baru',
            'description' => 'Formulir untuk pendaftaran anggota baru Wacana Style.',
            'status' => 'open',
            'starts_at' => now(),
            'ends_at' => null, // No end date for continuous registration
            'confirmation_message' => 'Terima kasih, pendaftaran Anda telah diterima. Admin akan segera menghubungi Anda.',
            'email_notification_enabled' => true,
            'admin_notification_enabled' => true,
        ]);
        ...
        if ($memberRegistrationForm->fields()->count() === 0) {
            ...
            foreach ($memberFields as $field) {
                FormField::create(array_merge(['form_id' => $memberRegistrationForm->id], $field));
            }
        }
    }
}
